...
// export interface ICreatePostRequest extends IUserIdRequest {
//   body: {
//     text: string;
//     description: string;
//   };
...
export class PostController {
  ...
  async createPost(
    ...
    try {
      // service
      const createPostResult = await this.postService.createPostService(
        // req,
        typedReq
      );

      return res.status(200).json(createPostResult);
    } catch (err) {
      return res.status(500).json({ error: "Не удалось создать пост." });
    }
  }

  async getallauthorposts(req: IUserIdRequest, res: Response) {
    // const userId = req.userId; // Получение ID пользователя
    try {
      //service
      const getallauthorpostsResult =
        await this.postService.getallauthorpostsService(req);
      return res.status(200).json(getallauthorpostsResult);
    } catch (err) {
      return res.status(500).json({ error: "Не удалось найти посты автора." });
    }
  }

  async getonepost(req: IUserIdRequest, res: Response) {
    try {
      // service
      const getonepostResult = await this.postService.getonepostService(req);
      return res.status(200).json(getonepostResult);
    } catch (err) {
      return res.status(500).json({ error: "Не удалось найти один пост" });
    }
  }
  ...
  async getallposts(req: Request, res: Response) {
    try {
      // service
      const getallpostsResult = await this.postService.getallpostsService(req);
      return res.status(200).json(getallpostsResult);
    } catch (err) {
      return res.status(500).json({ error: "Failed to find posts." });
    }
  }

  async likepost(req: IUserIdRequest, res: Response) {
    try {
      const likepostResult = await this.postService.likepostService(req);
      return res.status(200).json(likepostResult);
    } catch (err) {
      return res
        .status(500)
  ...
  async commentpost(req: IUserIdRequest, res: Response) {
    try {
      const commentpostResult = await this.postService.commentpostService(req);
      return res.status(200).json(commentpostResult);
    } catch (err) {
      return res
        .status(500)
        .json({ error: "Не удалось создать комментарий для поста." });
    }
  }

  async subcommentpost(req: IUserIdRequest, res: Response) {
    try {
      const subcommentpostResult = await this.postService.subcommentpostService(
        req
      );
      return res.status(200).json(subcommentpostResult);
    } catch (err) {
      return res
        .status(500)
